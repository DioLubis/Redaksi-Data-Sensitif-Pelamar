...
def main() -> None:
    ...
    name = f"yolov5{args.model_size}_{experiment['name']}_seed{experiment['seed']}"
    ...
    command = [
        ...
        "--img", str(image_size), "--epochs", str(epochs),
    ...
    write_metadata(output_dir, {"backend": "yolov5", "model": model, "device": device, "image_size": image_size, "epochs": epochs, "batch_size": batch_size, "workers": workers, "config": config})
    started = time.perf_counter()
    run(command)
    write_metadata(
        output_dir,
        {"backend": "yolov5", "model": model, "device": device, "image_size": image_size, "epochs": epochs, "batch_size": batch_size, "workers": workers, "config": config, "training_time_s": time.perf_counter() - started, "peak_gpu_memory_mb": None},
    )

...
from common import load_config, resolve, select_device, write_metadata
...
def main() -> None:
    ...
    write_metadata(output_dir, {"backend": "yolo26", "model": model_name, "device": device, "config": config})
    try:
        import torch
    ...
    model.train(
        data=str(resolve(experiment["data"])), imgsz=experiment["image_size"], epochs=experiment["epochs"],
        batch=experiment["batch_size"], device=device, workers=experiment["workers"], optimizer=experiment["optimizer"],
    ...
    write_metadata(
        output_dir,
        {"backend": "yolo26", "model": model_name, "device": device, "config": config, "training_time_s": time.perf_counter() - started, "peak_gpu_memory_mb": peak_memory_mb},
    )
